  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Agent Runtime Card */}
        <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-between gap-3 shadow-sm">
          ...
          <div>
            {agentRuntime?.running ? (
              <button
                ...
                title="Stop Agent Runtime Scheduler"
                className="px-2.5 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/20 text-xs font-medium transition-colors cursor-pointer disabled:opacity-50 inline-flex items-center gap-1"
              >
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/20 text-xs font-medium transition-colors cursor-pointer disabled:opacity-50 inline-flex items-center gap-1"
              >
        ...
        {/* Model Server Card */}
        <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex items-center gap-3.5 shadow-sm">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
              modelRuntime?.ready
        ...
        <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex items-center gap-3.5 shadow-sm">
          ...
          <div className="min-w-0">
            <div className="text-[10px] font-mono text-zinc-500 uppercase">Native Engine</div>
            <div className="text-sm font-bold text-zinc-100 truncate">
              {overview?.llama_installed ? 'llama.cpp Ready' : 'llama.cpp Missing'}
            </div>
            <div className="text-[11px] text-zinc-400 font-mono truncate">
              {modelRuntime?.running ? `PID: ${modelRuntime.pid}` : 'Release/llama-server'}
            </div>
          </div>
        </div>

        {/* Air-Gapped Security Card */}
        <div className="p-4 rounded-2xl bg-zinc-900/80 border border-emerald-500/20 flex items-center gap-3.5 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shrink-0">
            <Shield className="w-5 h-5" />
          </div>
          <div className="min-w-0">
            <div className="text-[10px] font-mono text-emerald-500 uppercase">Isolation</div>
            <div className="text-sm font-bold text-zinc-100">Strict Air-Gapped</div>
            <div className="text-[11px] text-zinc-400 font-mono">Egress: 0.0 KB (Local)</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
        <div className="flex items-center gap-2 p-1 rounded-xl bg-zinc-900/90 border border-zinc-800/80 text-xs w-fit">
          <button
            type="button"
          ...
          >
            ...
            {activeAgents.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full text-[10px] bg-emerald-500/20 text-emerald-300 font-mono border border-emerald-500/30">
                {activeAgents.length}
              </span>
            )}
          </button>
          ...
          >
            ...
            <span>Model Runtime Status</span>
            {modelRuntime?.ready && (
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 text-xs font-medium transition-colors cursor-pointer self-start sm:self-auto"
        >
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-zinc-900/80 border border-zinc-800 space-y-4 shadow-sm">
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${
                          agentRuntime?.running ? 'bg-emerald-400 animate-pulse' : 'bg-zinc-500'
                        }`}
                      />
              ...
              <div className="flex items-center gap-2 shrink-0">
                {agentRuntime?.running ? (
                  <button
                    ...
                    disabled={isTogglingAgentRuntime}
                    onClick={handleStopAgentRuntime}
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/20 text-xs font-semibold transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-zinc-800/80 font-mono text-xs">
              <div className="bg-zinc-950/50 p-2.5 rounded-xl border border-zinc-800/60">
                ...
                <span className={`font-semibold ${agentRuntime?.running ? 'text-emerald-400' : 'text-zinc-400'}`}>
              ...
              <div className="bg-zinc-950/50 p-2.5 rounded-xl border border-zinc-800/60">
                ...
                <span className="text-zinc-200">{agentRuntime?.loop_interval_seconds || 60}s cycle</span>
              </div>
              <div className="bg-zinc-950/50 p-2.5 rounded-xl border border-zinc-800/60">
                <span className="text-[10px] text-zinc-500 block uppercase">Active Workers</span>
                <span className="text-cyan-400 font-semibold">{activeAgents.length} Running</span>
              </div>
              <div className="bg-zinc-950/50 p-2.5 rounded-xl border border-zinc-800/60">
                <span className="text-[10px] text-zinc-500 block uppercase">Scheduler Thread</span>
                <span className="text-zinc-200">{agentRuntime?.scheduler_alive ? 'Alive' : 'Dead'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-zinc-900/70 border border-zinc-800 space-y-4">
            ...
            {isLoadingAgents ? (
              ...
              <div className="py-12 rounded-xl bg-zinc-950/60 border border-dashed border-zinc-800/80 text-center space-y-2">
                <Bot className="w-8 h-8 text-zinc-600 mx-auto" />
                <div className="text-xs font-semibold text-zinc-300">No Active Agent Threads</div>
              ...
              <div className="overflow-x-auto rounded-xl border border-zinc-800 bg-zinc-950">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-zinc-800 bg-zinc-900/60 font-mono text-[11px] text-zinc-400 uppercase">
                      ...
                      <tr key={item.id} className="hover:bg-zinc-900/40 transition-colors">
                        ...
                        <td className="py-3 px-4 text-right">
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/20 text-xs font-medium transition-colors cursor-pointer disabled:opacity-50 inline-flex items-center gap-1"
                          >
                            <Square className="w-3 h-3 fill-current" />
            ...
            {modelRuntime?.running && (
              <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-zinc-800/80 text-xs font-mono">
                ...
                <div className="flex items-center gap-3 text-zinc-400 text-[11px]">
                  ...
                  <span>GPU Offload: <b className="text-zinc-200">{modelRuntime.n_gpu_layers} layers</b></span>
            ...
            <div className="p-6 rounded-2xl bg-zinc-900/70 border border-zinc-800 space-y-4 flex flex-col justify-between">
              ...
              {!modelRuntime?.ready && (
                <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800 text-[11px] text-zinc-500">
                  Model daemon must be started and reported <b className="text-emerald-400">READY</b> to run live diagnostic inference.
                </div>
              )}
            </div>
          </div>
